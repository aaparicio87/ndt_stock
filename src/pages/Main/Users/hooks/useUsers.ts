...
export const useUser = (): IUseUser => {
  ...
  const [staffElement, setStaffElement] = React.useState<TStaff | undefined>(
    undefined,
  );
  const [staffElementDelete, setStaffElementDelete] = React.useState<
    string | undefined
  >(undefined);
  ...
  >(null);
  const [certificatesDetails, setCertificatesDetails] = React.useState<
    string[]
  >([]);
      ...
      if (staffData) {
        const staffDataTable = staffData.map((staff) => {
          const staffTableData: IStaffTable = {
            ...
            roles: staff.roles
              .map((role) => CAPITALIZED_ROLES[role])
              .join(", "),
            uid: staff.uid,
          };
  ...
  const handleGetAllCertificates = async () => {
    try {
      ...
      openToast(
        "error",
        `Error fetching staff: ${(error as Error).message}`,
        "Error",
      );
    }
  };
      ...
      if (uid) {
        ...
        if (staff !== null) {
          ...
          if (certificates && staff.certificates) {
            const result = getUserCertificatesName(
              staff.certificates,
              certificates,
            );
      ...
      if (uid) {
        ...
        if (staff !== null) {
          ...
          if (staff.certificates && certificatesRef.current) {
            const response = getUserCertificatesEdit(
              staff.certificates,
  ...
  const handleConfirmDelete = async () => {
    try {
      if (staffElementDelete) await deleteStaffElement(staffElementDelete);
      onCloseDelete();
  ...
  const onChangeItemCertificates = (data: MultiValue<TOptions>) => {
    ...
    if (certificatesRef.current) {
      ...
      const dataTransformed = transformSelectedElements(
        certificatesRef.current,
        certsSelecetd,
      );
