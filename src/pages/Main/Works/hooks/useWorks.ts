...
export interface IWorkHook {
  ...
  clearErrors: UseFormClearErrors<TWork>;
}
...
export const useWorks = (): IWorkHook => {
  ...
  const [workElementDelete, setWorkElementDelete] = React.useState<
    TWork | undefined
  >(undefined);
  const [workElementDetail, setWorkElementDetail] = React.useState<
    IDetailWork | undefined
  >(undefined);
  const [itemsCertificates, setItemsCertificates] = React.useState<
    MultiValue<TOptions>
  >([]);
  const [workersSelected, setWorkersSelected] = React.useState<
    MultiValue<TOptions>
  >([]);
  ...
  const handleChangeItemCertificates = (newValue: MultiValue<TOptions>) => {
    try {
      ...
      const typeWork = certificatesRemote.current.filter((item) =>
        newValue.map((data) => data.value as string).includes(item.uid ?? ""),
      );
      ...
      const workersLst = workersRemote.current
        .filter((wr) =>
          wr.certificates?.some((cert) => typeWorkIds.includes(cert.uid)),
        )
        .map((res) => ({
          label: `${capitalizeFirstLetter(res.name)} ${capitalizeFirstLetter(res.lastName)}`,
          value: `${res.uid}`,
  ...
  const handleChangeWorkersSelect = (newValue: MultiValue<TOptions>) => {
    ...
    const workersToSave = workersRemote.current.filter((item) =>
      dataArray.map((data) => data.value as string).includes(item.uid ?? ""),
    );
    setValue("workers", workersToSave);
  };

  const handleChangeCustomersSelect = (
    event: ChangeEvent<HTMLSelectElement>,
  ) => {
    const selectedValue = event.target.value;
    const client = customersRemote.current.find(
      (cr) => cr.uid === selectedValue,
    );
  ...
  const getWorkToUpdate = async (uid: string) => {
    try {
      ...
      if (work) {
        ...
        const tWork = work.typeWork
          .filter((tw) => tw.uid)
          .map((w) => ({ label: w.name, value: w.uid as string }));
        const workers = work.workers.map((w) => ({
          label: w.name,
          value: w.uid ?? "",
        }));
